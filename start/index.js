...
const client = makeWASocket({
    ...
    version: [2, 3000, 1023223821] // ✅ Ajout pour corriger l'erreur de connexion
});

    if (usePairingCode && !client.authState.creds.registered) {
        const phoneNumber = await question('please enter your WhatsApp number, starting with 509:\n');
  ...
  for (const user of participants) {
    if (action === 'remove') {
      ...
      await client.sendMessage(id, {
        image: { url: pp },
        caption: `🎉 *Goodbye @${user.split('@')[0]}*\n📌 Group: *${name}*\n👥 Members: *${count}*📝 Description: ${desc}`,
        mentions: [user],
        contextInfo: {
          forwardingScore: 5,
          isForwarded: true,
        ...
        if (global.ghosted?.[from]?.[sender]) {
            ...
            if (!PROTECTED.includes(sender)) {
                try {
                    ...
                    console.log(`🕳️ [GHOST] Message supprimé de ${sender}`);
                    return; // Stop ici, pas de suite de traitement
                } catch (err) {
                    console.error(`❌ Erreur ghost suppression :`, err);
                }
            }
        }

        // ===============================
        // 🧠 DÉLÉGUER AU FICHIER DE COMMANDES
        // ===============================
